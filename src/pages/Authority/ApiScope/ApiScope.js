...
import {
  MENU_PARENT_LIST,
  MENU_LOAD_API_SCOPE_DRAWER,
  MENU_SHOW_DRAWER,
} from '../../../actions/menu';
import ApiScopeCrud from './ApiScopeCrud';
...
class Menu extends PureComponent {
  showDrawer = (menuId, menuName, menuCode) => {
    ...
    dispatch(
      ...
      dispatch(MENU_LOAD_API_SCOPE_DRAWER({ menuId }));
    });
  };
  ...
  handleBtnCallBack = payload => {
    ...
    if (btn.code === 'api_scope_setting') {
      this.showDrawer(keys[0], rows[0].name, rows[0].code);
    }
  };

  render() {
    const code = 'api_scope';
