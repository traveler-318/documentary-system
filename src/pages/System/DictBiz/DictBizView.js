...
import { DICT_DETAIL } from '../../../actions/dictbiz';
...
@Form.create()
class DictBizView extends PureComponent {
  componentWillMount() {
    const {
      dispatch,
      match: {
  ...
  handleEdit = () => {
    ...
    router.push(`/system/dictbiz/edit/${id}`);
  };

  render() {
    const {
      dictbiz: { detail, parentId },
    } = this.props;
