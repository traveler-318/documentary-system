...
import { DICT_INIT, DICT_SUBMIT, DICT_DETAIL, DICT_CLEAR_DETAIL } from '../../../actions/dictbiz';
...
@connect(({ dictbiz, loading }) => ({
  dictbiz,
  submitting: loading.effects['dictbiz/submit'],
}))
@Form.create()
class DictBizAdd extends PureComponent {
  componentWillMount() {
    const {
      dispatch,
      match: {
  ...
  render() {
    ...
    const backUrl = parentMode ? `/system/dictbiz/sub/${parentId}` : '/system/dictbiz';
