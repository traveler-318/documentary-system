...
class AllOrdersList extends PureComponent {
  ...
  renderLeftButton = () => {
    return (
      <>
      <Button icon="upload" type='primary' onClick={this.exportFile}>导出</Button>
      </>)
  };
  moreMenu = () => (
  ...
  handleEdit = (row) => {
    ...
    router.push(`/order/delivered/edit/${row.id}`);
  }
  ...
  handleShowLogistics = (data) => {
    ...
    router.push('/order/delivered/logisticsConfiguration');
