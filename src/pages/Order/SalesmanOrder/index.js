...
class AllOrdersList extends PureComponent {
  ...
  constructor(props) {
    ...
    this.state = {
      ...
      data:{},
      loading:false,
      params:localStorage.salesmanOrderParams ? JSON.parse(localStorage.salesmanOrderParams) : {
        size:10,
        current:1
      },
      tabCode:[],
      tabKey:sessionStorage.salesmanOrderTabKey ? sessionStorage.salesmanOrderTabKey : '0',
      selectedRows:[],
      productList:[],
      // 物流弹窗
    ...
    return (<>
      {/* 待审核 */}
        ...
        <Button type="primary" icon="plus" onClick={()=>{
          router.push(`/order/SalesmanOrder/add`);
        }}>添加</Button>
        <Button
          icon="menu-unfold"
  ...
  statusChange = (key) => {
    sessionStorage.salesmanOrderTabKey = key;
  ...
  handleShowLogistics = (data) => {
    ...
    router.push('/order/salesmanOrder/logisticsConfiguration');
